...
void AccountBase::onRemoteMethodCall(MemoryStream& stream)
{
	...
	switch(pMethod->methodUtype)
	{
		case 8:
		{
			AVATAR_BRIEF_INFO_LIST rspAvatarList_arg1;
			((DATATYPE_AVATAR_BRIEF_INFO_LIST*)pMethod->args[0])->createFromStreamEx(stream, rspAvatarList_arg1);
			uint64 rspAvatarList_arg2 = stream.readUint64();
			rspAvatarList(rspAvatarList_arg1, rspAvatarList_arg2);
			break;
		}
		case 5:
		{
			uint32 rspCreateAvatar_arg1 = stream.readUint32();
			rspCreateAvatar(rspCreateAvatar_arg1);
			break;
		}
		case 7:
		{
			uint32 rspEnterGame_arg1 = stream.readUint32();
			rspEnterGame(rspEnterGame_arg1);
			break;
		}
		case 6:
		{
			uint32 rspRemoveAvatar_arg1 = stream.readUint32();
			rspRemoveAvatar(rspRemoveAvatar_arg1);
			break;
		}
		case 9:
		{
