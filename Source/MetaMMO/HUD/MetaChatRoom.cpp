...
#include "HUD/MetaChatRoom.h"
#include "Scripts/ExEventData.h"
#include "MetaChatItem.h"
...
void UMetaChatRoom::InstallEvent()
{
	// 绑定远程回调方法， 注册了的事件在关卡销毁时一定要注销
	KBENGINE_REGISTER_EVENT("AcceptChatList", AcceptChatList);
}

void UMetaChatRoom::UnInstallEvents()
{
	KBENGINE_DEREGISTER_ALL_EVENT();
}

void UMetaChatRoom::ShowOrHideInput(bool IsShow)
{
	if (IsShow)
	{
...
void UMetaChatRoom::SendChatInfo()
{
	// 获取输入字符并清空输入框
...
void UMetaChatRoom::AcceptChatList(const UKBEventData* EventData)
{
	...
	{
		UMetaChatItem* ChatItem = CreateWidget<UMetaChatItem>(this, ChatItemClass);
