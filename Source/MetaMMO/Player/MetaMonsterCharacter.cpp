...
AMetaMonsterCharacter::AMetaMonsterCharacter()
{
...
void AMetaMonsterCharacter::Tick(float DeltaSeconds)
{
...
void AMetaMonsterCharacter::SetBaseHP(int32 InBaseHP)
{
	BaseHP = InBaseHP;
}

void AMetaMonsterCharacter::SetHP(int32 InHP)
{
	if (InHP > 0)
	{
		// 播放受伤动画
...
void AMetaMonsterCharacter::PlayDeath()
{
	// 动画由状态机自己判定HP 是否小于0来播放

	GetWorld()->GetTimerManager().SetTimer(DestroyHandle, this, &AMetaMonsterCharacter::MonsterDestroy, 5.f);

}

void AMetaMonsterCharacter::BeginPlay()
{
	Super::BeginPlay();
...
void AMetaMonsterCharacter::MonsterDestroy()
{
	Destroy();
}
