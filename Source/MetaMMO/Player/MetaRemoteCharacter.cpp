...
FName AMetaRemoteCharacter::GroundName(TEXT("Ground"));

AMetaRemoteCharacter::AMetaRemoteCharacter()
{
...
void AMetaRemoteCharacter::Tick(float DeltaSeconds)
{
...
void AMetaRemoteCharacter::SetBaseHP(int32 InBaseHP)
{
	if (InBaseHP < BaseHP)
	{
		// 判断是否当前血值比新的血值小
...
void AMetaRemoteCharacter::SetHP(int32 InHP)
{
...
void AMetaRemoteCharacter::BeginPlay()
{
	Super::BeginPlay();

	// 获取血条
	BloodBar = Cast<UMetaBloodBar>(BloodBarComponent->GetUserWidgetObject());
	// 设置名字和类型
	BloodBar->SetRole(RoleType, RoleName);

}

void AMetaRemoteCharacter::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	// 是否碰到地面
...
void AMetaRemoteCharacter::OnOverLapEnd(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	// 是否碰到地面
	if (OtherActor->GetFName().IsEqual(GroundName))
	{
